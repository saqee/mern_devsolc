    ...
    },
    {
      ...
      detectionFunctions: [
        function (node, nodeOld) {
          ...
          // Example: "10 pieces available"
          //          "99% claimed"
          return /\d+\s*(?:\%|pieces?|pcs\.?|pc\.?|ct\.?|items?)?\s*(?:available|workday|kasse|express|sold|tax|claimed|redeemed)|(?:last|final)\s*(?:article|item)/i.test(
            node.innerText
          )
        },
        function (node, nodeOld) {
          ...
          // Example: "10 Stück verfügbar"
          //          "99% eingelöst"
          return /\d+\s*(?:\%|stücke?|stk\.?)?\s*(?:verfügbar|verkauft|ursprünglich|abholung|werktage|kasse|rabatt|summe|artikelrabatt|eingelöst)|letzter\s*Artikel/i.test(
            node.innerText
          )
        },
      ],
