...
async function initPatternHighlighter() {
  const activationState = await brw.runtime.sendMessage({
...
function resetDetectedPatterns() {
  let regx = new RegExp(
    "\\b" + constants.extensionClassPrefix + "[^ ]*[ ]?\\b",
    "g"
  )
  document
    .querySelectorAll("." + constants.patternDetectedClassName)
  ...
  for (const pattern of constants.patternConfig.patterns) {
    ...
    for (const elem of document.getElementsByClassName(
      ...
      if (elementIsVisible(elem)) {
        ...
        visibleText.push(elem.textContent)
      } else {
...
function sendResults() {
  ...
  console.log(
    brw.i18n.getMessage("infoNumberPatternsFound", [
      results.countVisible.toString(),
    ])
  )
}
...
function showElement(phid) {
  for (const element of document.getElementsByClassName(
    constants.currentPatternClassName
  )) {
    element.remove()
  }
